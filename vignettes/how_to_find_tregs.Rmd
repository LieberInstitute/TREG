...
For each group (let's use `cellType.broad`) get the proportion of zeros for each gene, where Proportion Zero is defined as (Equation \@ref(eq:propZero)) where $c_{i,j,k,z}$ is the number of snRNA-seq counts for cell/nuclei $z$ for gene $i$, cell type $j$, brain region $k$, and $n_{j, k}$ is the number of cell/nuclei for cell type $j$ and brain region $k$.
...
```{r "pick_cutoff", fig.cap = "Show Proption Zero Cutoff on distributions."}

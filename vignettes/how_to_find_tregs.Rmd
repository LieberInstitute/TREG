...
we will focus on the six major cell types listed in \@ref(tab:acronyms). 
...
get good results. This new cell type classification is stored in `cellType.broad`. 
...
```{r "top50FilterHeatmap", fig.cap = "Figure 1: Heatmap of the snRNA-seq counts. With top 50% filtering the data becomes less sparse."}
...
Proportion Zeros by group (Figure \@ref(fig:propZeroDistribution)).

```{r "propZeroDistribution", fig.cap = "Figure 2: Distribution of Proption Zero across cell types and regions."}
...
Looks like around 0.9 the densities peak, we'll set that as the cutoff (Figure \@ref(fig:pickCutoff)).

```{r "pickCutoff", fig.cap = "Figure 3: Show Proportion Zero Cutoff on distributions."}
...
One last check of the sparsity and heatmap (Figure \@ref(fig:propZeroFilterHeatmap)).

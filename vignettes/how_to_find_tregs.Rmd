...
The `r Biocpkg('TREG')` `r Citep(bib[['TREG']])` package was developed for identifying candidate TREGs for TODO by researchers at the Lieber Institute for Brain Development (LIBD) `r Citep(bib[['TREGpaper']])`. 
...
TODO describe a bit more what the example will cover
...
Here we download a public data snRNA-seq data that we'll use as our example `r Citep(bib[['tran2021']])`. 
...
Excit, Inhib subtypes (TODO define acronyms). 
...
```{r "examineSparsity", fig.cap = "TODO."}
...
and between groups is considered. One way to calculate RI is to dine the group 
